...
（インストール不要。自分のAPIキーで OpenAI / Gemini / Anthropic をすぐ試せます）

> 💡 **ローカルAI（Ollama）を使いたい場合**は、この公開版ではブラウザの制限で接続できません。
> このリポジトリをクローンして自分のPCで起動してください（下記セットアップ手順）。手元で動かせば業務データは一切外に出ません。
...
「JSON読込」は、従来の配列形式または`{ version: 1, flows: [...] }`形式のバックアップに加え、業務フライトレコーダーが出力する`workflow-flight-recorder` version 1を受け付けます。

フライトレコーダーJSONは、形式、version、工程・線の構造、重複・参照切れ、循環、BPMN XMLで安全でないIDを変換前に検証します。検証または変換に失敗した場合は保存しません。実演から分岐条件を推測してgatewayを足すこともありません。必要な分岐は、取り込み後に人が確認して編集してください。
